...
# 2. Download Tusk Engine
$TuskZip = "$TuskHome\tusk.zip"
$TuskUrl = "https://github.com/tusk-framework/tusk-engine/releases/latest/download/tusk_Windows_x86_64.zip"

Write-Host "Downloading Tusk Engine..." -ForegroundColor Yellow
try {
    Invoke-WebRequest -Uri $TuskUrl -OutFile $TuskZip -ErrorAction Stop
    Write-Host "Extracting Tusk Engine..." -ForegroundColor Yellow
    Expand-Archive -Path $TuskZip -DestinationPath $BinDir -Force
    Remove-Item $TuskZip
}
catch {
